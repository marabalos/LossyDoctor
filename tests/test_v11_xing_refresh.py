...
class V11XingRefresh(unittest.TestCase):
 ...
 def test_positive_fixture_exposes_metadata_only_mismatches(self):
  codes={i.code for i in analyze(self.d/'01_ffmpeg_xing_coupled_metadata_corrupt.mp3')['issues']};self.assertEqual(codes,{'XING_BYTE_COUNT_MISMATCH','XING_KIND_MISMATCH','XING_TOC_MISMATCH','XING_MUSIC_LENGTH_MISMATCH','XING_TAG_CRC_MISMATCH'})
 ...
 def test_refresh_diff_contains_only_coupled_xing_fields(self):
  a=self.A('01_ffmpeg_xing_coupled_metadata_corrupt.mp3');e=next(e for e in a.repair_execution if e.get('status')=='CREATED');fields={r['field'] for r in e['manifest']['changed_byte_ranges']};self.assertEqual(fields,{'XING_KIND','XING_BYTE_COUNT','XING_TOC','XING_MUSIC_LENGTH','XING_TAG_CRC'});self.assertFalse(e['manifest']['audio_recoding'])
 def test_payload_corruption_crc_mismatch_is_report_only(self):
  source=self.d/'payload_corrupt.mp3';source.write_bytes((self.d/'00_healthy_ffmpeg_xing.mp3').read_bytes());m=analyze(source);f=m['frames'][1];data=bytearray(source.read_bytes());data[f['byte_end']-1]^=1;source.write_bytes(data);before=sha256_file(source)
  row=self.A(source.name);self.assertIn('XING_AUDIO_CRC_MISMATCH',[i.code for i in row.issues]);plans=[p for p in row.repair_plan if p['spec']['id']=='REFRESH_XING_METADATA'];self.assertTrue(plans and plans[0]['status']=='BLOCKED');self.assertFalse(any(x.get('status') in ('CREATED','REUSED') for x in row.repair_execution));self.assertEqual(sha256_file(source),before)
