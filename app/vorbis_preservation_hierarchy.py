...
POLICY = 'VORBIS_PRESERVATION_HIERARCHY_STRICT_V1'
ORDER = [
...
_ALLOWED_RECOVERY_KIND = 'RECOVERED_VORBIS_PROVEN_REGION_LOSSLESS'
...
def resolve(
    ...
    """Resuelve la familia de preservación Vorbis realmente publicada o reutilizada.

    Es sólo observacional: ya se ejecutaron la elegibilidad, la verificación de
    reparaciones y la verificación de recuperación sin pérdida. Este resolutor no
    concede autoridad nueva; explicita la precedencia y falla de forma cerrada si
    coexisten familias incompatibles.
    """
    ...
    recovery_tier = None
    if recovery_outputs:
        # Familia a nivel de origen. Una recuperación en varias partes puede contener un prefijo que
        # no alcanza EOS y una cola que sí. Si alguna región publicada
        # alcanza un EOS autenticado por CRC; entonces la familia del origen es Tier 2.
        has_authenticated_eos = any(
            bool((o.get('manifest') or {}).get('includes_authenticated_eos'))
            and (o.get('manifest') or {}).get('temporary_eos_trim_samples') is not None
            for o in recovery_outputs
        )
        recovery_tier = ORDER[1] if has_authenticated_eos else ORDER[2]
    ...
    if selected == ORDER[0]:
        reason = 'la recaptura verificada de páginas Ogg/Vorbis autenticadas por CRC superó la validación estructural y de decodificación completa posterior; se suprimen todas las alternativas PCM'
    elif selected == ORDER[1]:
        reason = 'ninguna reparación verificada de bitstream prevaleció; las regiones de paquetes Vorbis completos autenticados por CRC son recuperables y al menos una región publicada alcanza un EOS autenticado con recorte exacto delimitado por gránulos'
    elif selected == ORDER[2]:
        reason = 'los niveles superiores no están disponibles; sólo se publican regiones de paquetes Vorbis completos autenticados por CRC con cobertura abierta o truncada, sin afirmar una línea de tiempo completa'
    elif selected == ORDER[3]:
        reason = 'persiste una anomalía, pero la evidencia no autoriza una reparación segura del bitstream ni una salida de preservación PCM según la política Vorbis V1'
    else:
    ...
    if unknown_lossless:
        violation = 'UNKNOWN_VORBIS_PRESERVATION_DERIVATION_FAMILY'
    elif len(ranked) > 1:
        violation = 'MULTIPLE_VORBIS_PRESERVATION_TIERS_PUBLISHED_SIMULTANEOUSLY'
    else:
        violation = None
